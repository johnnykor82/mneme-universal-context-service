...
If two machines share project files through symlinked directories, do not assume
Mneme is installed on both machines just because the shared files are visible.
Shared files can carry docs and examples, but runtime pieces remain
per-machine:
...
- configure `mneme mcp` in that machine's host environment;
- set local tokens and provider secrets;
- review and trust any host-adapter hook config on that machine.
