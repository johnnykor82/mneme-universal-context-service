...
| Phase 13B final GitHub install verification | Inspect installed distribution `direct_url.json` after pushing planning notes | Ensure the global venv records a GitHub `main` installation rather than the local checkout | Installed distribution points to the current GitHub `main`; installed `TOOL_NAMES` include `resolve_session` and `list_sessions` | pass |
