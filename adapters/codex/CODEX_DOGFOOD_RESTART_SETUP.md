...
- `/Users/openclaw/.codex/config.toml` has been updated with `[mcp_servers.mneme]`.
- A backup exists at `/Users/openclaw/.codex/config.toml.bak-mneme-dogfood`.
...
```bash
cd /Users/openclaw/.hermes/plugins/_mneme-universal-context-service
...
Already added to `/Users/openclaw/.codex/config.toml`. The snippet is also saved in `adapters/codex/codex_mcp_dogfood.example.json`.
...
- confirm the config command points to `/Users/openclaw/.hermes/plugins/_mneme-universal-context-service/.venv/bin/python`;
- confirm `PYTHONPATH` is `/Users/openclaw/.hermes/plugins/_mneme-universal-context-service`;
